...
func TestModel(t *testing.T) {
	...
	t.Run("Sum Many", func(t *testing.T) {
		...
		sum, err := UserModel.SumMany(0, []string{"age"}, bson.M{})
		if err != nil {
			t.Error(err)
		}
	...
	t.Run("Sum", func(t *testing.T) {
		...
		sum, err := UserModel.Sum("age", bson.M{})
		if err != nil {
			t.Error(err)
		}

		assert.EqualValues(t, sum, 20)
	...
	// Test `PaginateAggregateRaw`
	t.Run("Paginate Aggregate Raw", func(t *testing.T) {
		CreateMultipleTestUsers(10)

		// paginate
		paginated, err := UserModel.PaginateAggregateRaw(1, 5, nil)
